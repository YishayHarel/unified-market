...
serve(async (req) => {
  ...
  try {
    ...
    // Use Lovable AI Gateway for personalized analysis
    const LOVABLE_API_KEY = Deno.env.get('LOVABLE_API_KEY');
    
    if (!LOVABLE_API_KEY) {
      console.error('[Morning Brief] LOVABLE_API_KEY not configured');
      return new Response(
...
Provide a comprehensive Morning Market Brief.`;
    ...
    console.log('[Morning Brief] Calling Lovable AI Gateway...');
    ...
    if (!response.ok) {
      ...
      console.error('[Morning Brief] AI Gateway error:', response.status, errorText);
