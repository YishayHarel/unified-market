...
serve(async (req) => {
  ...
  try {
    ...
    if (!LOVABLE_API_KEY) {
      console.error('[AI Daily Brief] LOVABLE_API_KEY not configured');
      return new Response(
...
Create a comprehensive but concise daily brief.`;
    ...
    console.log('[AI Daily Brief] Calling Lovable AI Gateway...');
    ...
    if (!response.ok) {
      ...
      console.error('[AI Daily Brief] AI Gateway error:', response.status, errorText);
